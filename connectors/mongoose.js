...
function nop_cleanup(number_of_people, callback) {
    if(number_of_people) {
        if (number_of_people.indexOf("שני") > -1 || number_of_people.indexOf("שתי") > -1 || location.substr(1) === '2') {
            return callback(2);
        }
        else if (number_of_people.indexOf("שלוש") > -1 || location.substr(1) === '3') {
            return callback(3);
        }
        else if (number_of_people.indexOf("ארבע") > -1 || location.substr(1) === '4') {
            return callback(4);
        }
        else if (number_of_people.indexOf("חמש") > -1 || number_of_people.indexOf("חמישה") > -1 || location.substr(1) === '5') {
            return callback(5);

        }
        else if (number_of_people.indexOf("שש") > -1 || number_of_people.indexOf("שישה") > -1 || location.substr(1) === '6') {
            return callback(6);

        }
        else if (number_of_people.indexOf("שבע") > -1 || location.substr(1) === '7') {
            return callback(7);

        }
        else if (number_of_people.indexOf("שמונה") > -1 || location.substr(1) === '8') {
            return callback(8);

        }
        else if (number_of_people.indexOf("תשע") > -1 || location.substr(1) === '9') {
            return callback(9);

        }
        else if (number_of_people.indexOf("עשר") > -1 || location.substr(1) === '10') {
            return callback(10);
